...
const Matcher isNotification = MatchesJsonObject('notification', {'event': isString},
...
abstract class AbstractFormatterServerIntegrationTest extends IntegrationTestMixin
{
    ...
    AbstractFormatterServerIntegrationTest()
    {
...
class MatchesJsonObject extends _RecursiveMatcher
{
    ...
    const MatchesJsonObject(this.description, this.requiredFields, {this.optionalFields});
    ...
    void _checkField(
        String key, value, Matcher valueMatcher, List<MismatchDescriber> mismatches)
    {
...
class Server
{
    ...
    /// Find the root directory of the formatter_server package by proceeding
    ...
    Future<Map<String, Object?>?> send(String method, Map<String, Object?>? params)
    {
        var id = '${_nextId++}';
    ...
    }) async
    {
        ...
        else
        {
            ...
            serverPath = path.normalize(path.join(rootDir, 'bin', 'listen.dart'));
        }
        ...
        _process = await Process.start(
            dartBinary,
            arguments,
            environment: {'DART_SERVER_DISABLE_PUB_COMMAND': 'true'},
        );
...
abstract class _RecursiveMatcher extends Matcher
{
    ...
    void checkSubstructure(item, Matcher matcher, List<MismatchDescriber> mismatches,
