...
''');
        ...
        expect(formatResult.edits, isNotNull);
        expect(formatResult.edits, hasLength(2));

        var edit = formatResult.edits[0];
        expect(edit.replacement, equals('\n    '));
        var edit2 = formatResult.edits[1];
        expect(edit2.replacement, equals('\n'));
...
''');
    ...
    //TODO(tekert) dart_polisher is already tested, check if we need more test here.
    Future<EditFormatResult> _formatAt(int selectionOffset, int selectionLength,
        {int? lineLength}) async
    {
        var request = EditFormatParams(testFile.path, selectionOffset, selectionLength,
