...
class CodegenProtocolVisitor extends DartCodegenVisitor with CodeGenerator
{
    ...
    CodegenProtocolVisitor(
        this.packageName, this.responseRequiresRequestTime, this.isServer, Api api)
    ...
    bool emitConvenienceConstructor(String className, ImpliedType impliedType)
    {
        ...
        indent(()
        {
            var fieldNameString = literalString(fieldName.replaceFirst(RegExp('^_'), ''));
    ...
    void emitImports()
    {
        //TODO (tekert): check why we are not using this jsonDecoder then remove this writeln
        writeln(
    ...
    void emitObjectClass(String className, TypeObject type, ImpliedType impliedType)
    {
        docComment(toHtmlVisitor.collectHtml(()
        {
            toHtmlVisitor.p(()
            ...
            if (isOptionalConstructorArg(className, field))
            {
                if (!field.optional)
                {
                    ...
                    if (fieldType is TypeList)
                    {
                        ...
                        initializers
                            .add('${field.name} = ${field.name} ?? $defaultValue');
                    }
                    else
                    {
