...
final GeneratedFile target = GeneratedFile(
    'test/integration/support/integration_test_methods.dart', (String pkgPath) async
{
    var visitor = CodegenInttestMethodsVisitor(path.basename(pkgPath), readApi(pkgPath));
...
class CodegenInttestMethodsVisitor extends DartCodegenVisitor with CodeGenerator
{
    ...
    {
        var streamName = camelJoin(['on', notification.domainName, notification.event]);
        var className = camelJoin([notification.domainName, notification.event, 'params'],
            doCapitalize: true);
        writeln();
        docComment(toHtmlVisitor.collectHtml(()
        {
    ...
    void visitRequest(Request request)
    {
        ...
        indent(()
        {
            var requestClass = camelJoin([request.domainName, request.method, 'params'],
                doCapitalize: true);
            var paramsVar = 'null';
            if (params != null)
            {
